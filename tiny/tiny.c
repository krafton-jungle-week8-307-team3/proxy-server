...
void doit(int fd) {
    ...
    /* 요청 라인과 헤더 읽기 */
    Rio_readinitb(&rio, fd); // 버퍼(rio)를 초기화 후, rio와 파일 디스크립터(fd)를 연결한다.
    ...
    if (strcasecmp(method, "GET")) {
      clienterror(fd, method, "501", "NOT IMPLEMENTED", "Tint does not implement this method");  // 요청 메소드가 GET이 아니면 에러 처리
      return;
    }
    read_requesthdrs(&rio);  // 요청 헤더를 읽고 파싱한다.(요청 헤더를 읽어서 클라이언트가 요청한 추가 정보를 처리한다.)

    /* URI 파싱 */
    is_static = parse_uri(uri, filename, cgiargs); // 요청이 static content or dynamic content 파악
    if (stat(filename, &sbuf) < 0 ) {
      clienterror(fd, filename, "404", "404 Not Found", "Tint does not exist");
      return;
    }

    if (is_static) {
      if (!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode)) {
        clienterror(fd, filename, "404", "403 Not Found", "Tiny could not find this file");
        return;
      }
...
void clienterror(int fd, char *cause, char *errnum, char *shortmsg, char *longmsg) {
  char buf[MAXLINE], body[MAXLINE]; // buf: HTTP 응답 헤더, body: HTML 응답의 본문인 문자열(오류 메시지와 함께 HTML 형태로 클라이언트에게 보여짐)
...
void read_requesthdrs(rio_t *rp) {
  ...
  Rio_readlineb(rp, buf, MAXLINE); // 요청 메시지의 첫 번째 줄 읽기
